...
export function computeOffset({
  ...
  const manualOffset = isMobile ? (__stickyOffset ?? 0) - (__mobileStickyOffset ?? 0) : __stickyOffset ?? 0;
  if (hasInnerOverflowParents) {
    return `${manualOffset}px`;
  }
  const globalOffset = `var(${globalVars.stickyVerticalTopOffset}, 0px)`;

  return `calc(${globalOffset} + ${manualOffset}px)`;
}
...
export const useStickyHeader = (
  ...
  // TODO: add extra spaceScaledS top padding
