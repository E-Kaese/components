  ...
  const columnDefinitions = [
    {
      ...
      label: (
        <Box margin={{ left: 'xxs' }}>
          <Checkbox checked={selection.ALL === 'on'} indeterminate={selection.ALL === 'partial'} onChange={toggleAll} />
        </Box>
      ),
    ...
    },
    {
      ...
      label: 'Account',
      render: (item: TransactionRow) =>
        typeof item.accountId === 'string' ? item.accountId : `${item.accountId} ${plural(item.accountId, 'account')}`,
    },
    {
      key: 'id',
    ...
    <ContentLayout
      header={
        <Box padding="m">
          <Header
            variant="h1"
            counter={selected ? `(${transactions.length} / ${selected})` : `(${transactions.length})`}
          >
            Transactions
          </Header>
        </Box>
